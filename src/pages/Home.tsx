...
export default function Home() {
  ...
  const [activePillar, setActivePillar] = useState<'education' | 'tourism' | 'trade'>('education');
  ...
  return (
    <>
      <main className="pt-0 transition-colors duration-300">
        {/* Hero Section */}
        <section className="min-h-[calc(100vh-72px)] flex flex-col md:flex-row overflow-hidden mesh-grain bg-white dark:bg-slate-950">
          {/* Left Content (55%) */}
            ...
            <div className="lg:col-span-4 flex flex-col gap-4">
              {pillars.map((pillar, idx) => (
                <button
                  ...
                  className={cn(
                    "flex items-center justify-between p-6 rounded-2xl transition-all duration-300 text-left",
              ...
              <div className="grid md:grid-cols-2 gap-12 items-center min-h-[400px]">
                <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
                  ...
                  <div className="flex flex-wrap gap-4 pt-4">
                    <button className="bg-primary text-on-primary px-8 py-3 rounded-xl font-bold hover:opacity-90 transition-all shadow-md active:scale-95">
                      {t(`home.expertise.${activePillar}_panel.cta`)}
                    </button>
                    <button className="text-primary dark:text-blue-400 font-mono text-sm font-bold flex items-center gap-2 group">
                      {t(`home.expertise.${activePillar}_panel.learn_more`)} <span className="material-symbols-outlined group-hover:translate-x-1 transition-transform">arrow_forward</span>
                    </button>
                  </div>
                </div>
        ...
        <section className="bg-slate-950 py-32 relative overflow-hidden">
          ...
          <div className="max-w-[1440px] mx-auto px-8 relative z-10 flex flex-col items-center text-center">
            <svg className="text-white/20 mb-12" fill="none" height="60" viewBox="0 0 24 24" width="60">
            ...
            <div className="flex flex-col items-center">
              <div className="w-20 h-20 rounded-full border-2 border-secondary p-1 mb-4">
                <img className="w-full h-full object-cover rounded-full" alt={t('home.testimonial.name')} src="/assets/images/testimonial_ananya.jpg" />
              </div>
              <div className="text-white font-bold text-xl">{t('home.testimonial.name')}</div>
              <div className="font-mono text-sm text-slate-400 uppercase tracking-widest mt-1">{t('home.testimonial.role')}</div>
            </div>
          </div>
        </section>
